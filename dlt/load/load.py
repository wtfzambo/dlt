    ...
    def get_new_jobs_info(
        ...
        for job_file in new_job_files:
            if (
                ...
                or self.get_load_table(schema, job_file)["write_disposition"] in dispositions
            ):
        ...
        with self.get_destination_client(schema) as job_client:
            ...
            if expected_update is not None:
                ...
                truncate_tables = self.get_table_chain_tables_for_write_disposition(
                    load_id, schema, job_client.get_truncate_destination_table_dispositions()
                )
                ...
                if isinstance(job_client, WithStagingDataset):
                    if staging_tables := self.get_table_chain_tables_for_write_disposition(
                        ...
                        with job_client.with_staging_dataset():
                            logger.info(
                                f"Client for {job_client.config.destination_name} will start"
                                " initialize STAGING storage"
                            )
                            job_client.initialize_storage()
                            logger.info(
                                f"Client for {job_client.config.destination_name} will UPDATE"
                                " STAGING SCHEMA to package schema"
                            )
                            job_client.update_storage_schema(
                                only_tables=staging_tables | {schema.version_table_name},
                                expected_update=expected_update,
                            )
                            logger.info(
                                f"Client for {job_client.config.destination_name} will TRUNCATE"
                                f" STAGING TABLES: {staging_tables}"
                            )
