...
def test_evolve_schema(destination_config: DestinationTestConfiguration) -> None:
    ...
    assert_query_data(
        p, "SELECT schema_version_hash FROM _dlt_loads ORDER BY inserted_at", version_history
    )
...
@pytest.mark.parametrize(
    "destination_config",
    destinations_configs(default_sql_configs=True, subset=["bigquery", "snowflake", "duckdb"]),
    ids=lambda x: x.name,
)
def test_parquet_loading(destination_config: DestinationTestConfiguration) -> None:
    ...
    with client.sql_client as sql_client:
        assert [row[0] for row in sql_client.execute_sql("SELECT * FROM other_data")] == [
            1,
            2,
            3,
            4,
        ...
        assert_all_data_types_row(
            db_row[:-2],
            parse_complex_strings=destination_config.destination in ["snowflake", "bigquery"],
        )
