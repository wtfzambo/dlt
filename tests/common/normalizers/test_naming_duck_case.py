...
def test_alphabet_reduction(naming_unlimited: NamingConvention) -> None:
    assert (
        naming_unlimited.normalize_identifier(NamingConvention._REDUCE_ALPHABET[0])
        == NamingConvention._REDUCE_ALPHABET[1]
    )
...
def test_duck_snake_case_compat(naming_unlimited: NamingConvention) -> None:
    ...
    assert (
        snake_unlimited.normalize_identifier(NamingConvention._REDUCE_ALPHABET[0])
        == NamingConvention._REDUCE_ALPHABET[1]
    )
    # but there are differences in the reduction
    assert naming_unlimited.normalize_identifier(
        SnakeNamingConvention._REDUCE_ALPHABET[0]
    ) != snake_unlimited.normalize_identifier(SnakeNamingConvention._REDUCE_ALPHABET[0])
